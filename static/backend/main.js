...
(function () {
  ...
  var SIDEBAR_ITEMS = [
    ...
    { view: 'alerts', label: 'AI Alerts', icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg>' },
    { view: 'chat', label: 'Manager Chat', icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/></svg>' },
    ...
    { view: 'agent', label: 'Agent Autopilot', icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/></svg>' }
  ];
  ...
  function setView(viewName) {
    ...
    if (DOM.alertsPanel) {
      DOM.alertsPanel.classList.toggle('hidden', state.currentView !== 'alerts');
    }
    if (DOM.agentPanel) {
  ...
  function fetchHomeSummary() {
    fetch('/api/home-summary')
      ...
      .catch(function (err) { console.error('Home summary fetch failed:', err); });
  }
  ...
  function renderOutlookAlerts(items) {
    ...
    var html = visible.map(function (item, idx) {
      ...
      var barClass = isStockout ? 'bg-red-500 shadow-[0_0_15px_rgba(239,68,68,0.5)]' : 'bg-yellow-500';
      ...
      return '<div class="glass-panel p-8 rounded-[32px] hover:border-white/20 transition-all duration-500 group relative overflow-hidden flex flex-col justify-between border-white/5" ' + animationDelay + '>' +
        ...
        '<div><h3 class="text-xl font-bold tracking-tight mb-2 group-hover:text-indigo-400 transition-colors">' + escapeHtml(item.item) + '</h3>' +
        '<div class="flex items-center gap-2">' +
        '<span class="px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider border transition-all duration-300 mono ' + badgeClass + '">' + eventLabel + '</span>' +
        '<span class="text-[9px] mono text-white/20 uppercase tracking-widest">' + (item.days_until || '?') + ' DAYS OUT</span></div></div>' +
        '<div class="text-4xl font-black tracking-tighter mono ' + pctClass + '">' + pct + '<span class="text-sm opacity-20">%</span></div></div>' +
        '<div class="relative z-10 mt-10 space-y-5">' +
        '<p class="text-xs text-white/40 leading-relaxed italic border-l border-white/10 pl-4 py-1">"AI confidence: ' + pct + '% — ' + eventLabel.toLowerCase() + ' predicted within ' + (item.days_until || '?') + ' days"</p>' +
        '<div class="flex items-center justify-between"><div class="flex-1 h-1 bg-white/5 rounded-full overflow-hidden mr-6">' +
        '<div class="h-full transition-all duration-1000 ' + barClass + '" style="width: ' + pct + '%"></div></div>' +
    ...
    var moreLabel = moreCount > 0 ? moreCount + ' MORE ALERTS' : 'FULL AI ALERTS VIEW';
    html += '<div class="glass-panel p-8 rounded-[32px] border-indigo-500/20 flex flex-col items-center justify-center cursor-pointer hover:bg-indigo-500/10 hover:border-white/20 transition-all duration-500 group relative overflow-hidden view-more-btn" data-target="alerts">' +
      ...
      '<p class="text-[9px] mono uppercase tracking-widest text-white/30 mt-1">' + moreLabel + '</p></div></div></div>';
  ...
  function loadAlerts() {
    ...
    if (DOM.alertsResults && !DOM.alertsResults.classList.contains('hidden')) {
      fetch('/alerts')
        ...
        .catch(function (err) { console.error('Load alerts failed:', err); });
    }
  }
  ...
  function renderAlerts(alerts) {
    ...
    DOM.alertsList.innerHTML = alerts.map(function (a) {
      var ev = a.risk_event || {};
      var isStockout = ev.event_type === 'STOCKOUT_RISK';
      var borderColor = isStockout ? 'border-red-500/30' : 'border-amber-500/30';
      var tagColor = isStockout ? 'text-red-400 bg-red-500/10 border-red-500/20' : 'text-amber-400 bg-amber-500/10 border-amber-500/20';
      var glowBg = isStockout ? 'bg-red-600' : 'bg-amber-600';
      var actions = (a.suggested_actions || []).map(function (act) {
        return '<li class="flex items-start gap-2"><span class="text-indigo-400 mt-0.5">→</span><span>' + escapeHtml(act) + '</span></li>';
      }).join('');
      var ctx = a.historical_context || {};
      var ctxInfo = '';
      if (ctx.avg_daily_use) {
        ctxInfo = '<div class="flex flex-wrap gap-3 mt-3">' +
          '<span class="text-[9px] mono text-white/30 px-2 py-1 bg-white/5 rounded-lg">Avg use: ' + ctx.avg_daily_use + '/day</span>' +
          '<span class="text-[9px] mono text-white/30 px-2 py-1 bg-white/5 rounded-lg">Avg waste: ' + ctx.avg_daily_waste + '/day</span>' +
          '<span class="text-[9px] mono text-white/30 px-2 py-1 bg-white/5 rounded-lg">Trend: ' + escapeHtml(ctx.trend || 'n/a') + '</span>' +
          '<span class="text-[9px] mono text-white/30 px-2 py-1 bg-white/5 rounded-lg">Covers (7d): ' + (ctx.last_week_covers || 0) + '</span>' +
          '</div>';
      }
      return '<div class="glass-panel p-6 ' + borderColor + ' relative overflow-hidden hover:border-white/20 transition-all duration-300">' +
        '<div class="absolute -right-8 -bottom-8 w-24 h-24 blur-[50px] opacity-10 rounded-full ' + glowBg + '"></div>' +
        '<div class="relative z-10">' +
        '<div class="flex items-start justify-between mb-3">' +
        '<div class="flex items-center gap-3">' +
        '<span class="px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider border mono ' + tagColor + '">' + (ev.event_type || '').replace('_', ' ') + '</span>' +
        '<h3 class="text-lg font-bold tracking-tight">' + escapeHtml((ev.item_id || '').replace(/_/g, ' ')) + '</h3>' +
        '</div>' +
        '<div class="flex items-center gap-4 text-right">' +
        '<div><span class="text-[9px] mono text-white/30 block">Confidence</span><span class="text-xl font-black mono">' + Math.round((ev.confidence || 0) * 100) + '%</span></div>' +
        '<div><span class="text-[9px] mono text-white/30 block">Days</span><span class="text-xl font-black mono">' + (ev.days_until || '?') + '</span></div>' +
        '</div></div>' +
        '<p class="text-sm text-white/70 leading-relaxed mb-3">' + escapeHtml(a.alert_message || '') + '</p>' +
        (actions ? '<ul class="text-xs text-white/60 space-y-1.5 mb-2">' + actions + '</ul>' : '') +
        ctxInfo +
        '</div></div>';
    }).join('');
  }

  var spinnerSvg = '<svg class="animate-spin h-5 w-5 text-black" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" fill="none"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>';
  var boltSvg = '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>';
  var boltSvgLg = '<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>';
  ...
  function handleRunCheck(isRescan) {
    ...
    if (DOM.alertsScanStatus) DOM.alertsScanStatus.textContent = 'Scanning pipeline...';

    fetch('/run-inventory-check', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' })
      .then(function (res) { return res.ok ? res.json() : Promise.reject('Request failed'); })
      .then(function (data) {
        var sum = data.pipeline_summary || {};
        if (DOM.sumPredictions) DOM.sumPredictions.textContent = sum.predictions_loaded || 0;
        if (DOM.sumRiskEvents) DOM.sumRiskEvents.textContent = sum.risk_events_generated || 0;
        if (DOM.sumAfterRules) DOM.sumAfterRules.textContent = sum.after_rule_engine || 0;
        if (DOM.sumAlertsSent) DOM.sumAlertsSent.textContent = sum.eligible_for_alert || sum.alerts_generated || 0;
      ...
      .catch(function (err) {
        ...
        if (DOM.alertsScanStatus) DOM.alertsScanStatus.textContent = 'Scan failed — check server logs';
        if (DOM.alertsList) DOM.alertsList.innerHTML =
          '<div class="glass-panel p-6 text-center text-red-400 text-sm">Error running inventory check. Check server logs.</div>';
      })
      .finally(function () {
        btn.disabled = false;
        if (label) label.textContent = isRescan ? 'Re-Scan' : 'Run Inventory Check';
  ...
  function appendChatMessage(role, text) {
    ...
    var html = '<div class="flex items-start gap-3 ' + justifyClass + '">' +
      (isUser ? '' : avatar) +
      '<div class="' + bubbleClass + ' px-4 py-3 max-w-[80%]"><p class="text-sm text-white/80 leading-relaxed">' + formatted + '</p></div>' +
      (isUser ? avatar : '') +
      '</div>';
  ...
  function appendChatLoading() {
    if (!DOM.chatMessages) return;
    var html = '<div class="flex items-start gap-3 chat-loading">' +
  ...
  function handleChatSend(message) {
    ...
    fetch('/chat', {
      ...
      .catch(function (err) {
        removeChatLoading();
        appendChatMessage('ai', 'Sorry, I couldn\'t process that. Please try again.');
  ...
  if (DOM.btnRunCheck) {
    DOM.btnRunCheck.addEventListener('click', function () { handleRunCheck(false); });
  }
  if (DOM.btnRunCheckAgain) {
    DOM.btnRunCheckAgain.addEventListener('click', function () { handleRunCheck(true); });
  }
  ...
  if (DOM.chatQuickQuestions) {
    DOM.chatQuickQuestions.querySelectorAll('.chat-chip').forEach(function (chip) {
  ...
  function agentStatusBadge(status) {
    ...
    return '<span class="inline-block text-[9px] mono uppercase tracking-widest px-2.5 py-1 rounded-full border ' + cls + '">' + (status || '—').replace('_', ' ') + '</span>';
  }
  ...
  function agentRiskBadge(level) {
    ...
    return '<span class="inline-block text-[9px] mono uppercase tracking-widest px-2.5 py-1 rounded-full border ' + cls + '">' + (level || '—') + '</span>';
  }
  ...
  function agentTypeBadge(type) {
    ...
    return '<span class="text-[10px] mono text-white/70">' + (labels[type] || type || '—') + '</span>';
  }
  ...
  function agentActionButtons(action) {
    ...
    if (action.status === 'proposed') {
      btns.push('<button onclick="agentApprove(\'' + id + '\')" class="text-[9px] mono uppercase tracking-widest px-2.5 py-1 rounded-full border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/10 transition-all">Approve</button>');
      btns.push('<button onclick="agentReject(\'' + id + '\')" class="text-[9px] mono uppercase tracking-widest px-2.5 py-1 rounded-full border border-red-500/30 text-red-400 hover:bg-red-500/10 transition-all">Reject</button>');
    }
    if (action.status === 'approved') {
      btns.push('<button onclick="agentExecute(\'' + id + '\')" class="text-[9px] mono uppercase tracking-widest px-2.5 py-1 rounded-full border border-indigo-500/30 text-indigo-400 hover:bg-indigo-500/10 transition-all">Execute</button>');
    }
    if (action.status === 'executed') {
      btns.push('<button onclick="agentRollback(\'' + id + '\')" class="text-[9px] mono uppercase tracking-widest px-2.5 py-1 rounded-full border border-gray-500/30 text-gray-400 hover:bg-gray-500/10 transition-all">Rollback</button>');
    }
    // All rows get a Details button
    btns.push('<button onclick="agentShowDiff(\'' + id + '\')" class="text-[9px] mono uppercase tracking-widest px-2.5 py-1 rounded-full border border-white/10 text-white/40 hover:bg-white/5 transition-all">Details</button>');
    return '<div class="flex items-center gap-1.5 flex-wrap">' + btns.join('') + '</div>';
  }
  ...
  function renderAgentQueue() {
    ...
    if (fs) filtered = filtered.filter(function (a) { return a.status === fs; });
    if (fo) filtered = filtered.filter(function (a) { return a.owner_role === fo; });
    ...
    // Stats
    if (DOM.agentStatTotal)    DOM.agentStatTotal.textContent = agentState.actions.length;
    if (DOM.agentStatProposed) DOM.agentStatProposed.textContent = agentState.actions.filter(function (a) { return a.status === 'proposed'; }).length;
    if (DOM.agentStatExecuted) DOM.agentStatExecuted.textContent = agentState.actions.filter(function (a) { return a.status === 'executed'; }).length;
    if (DOM.agentStatAlerts)   DOM.agentStatAlerts.textContent = new Set(agentState.actions.map(function (a) { return a.alert_id; })).size;

    if (!DOM.agentQueueBody) return;
    if (filtered.length === 0) {
      DOM.agentQueueBody.innerHTML = '<tr><td colspan="7" class="text-center text-white/20 py-8">No actions match the current filters.</td></tr>';
      return;
    }
    DOM.agentQueueBody.innerHTML = filtered.map(function (a) {
      var ingredient = '—';
      if (a.params) ingredient = a.params.ingredient || a.params.item || '—';
      return '<tr class="agent-row hover:bg-white/[0.02] cursor-pointer transition-colors">' +
        '<td>' + agentTypeBadge(a.action_type) + '</td>' +
        '<td class="text-[10px] mono text-white/60">' + ingredient + '</td>' +
        '<td>' + agentRiskBadge(a.risk_level) + '</td>' +
        '<td class="text-[10px] mono text-white/50">' + (a.owner_role || '—') + '</td>' +
        '<td>' + agentStatusBadge(a.status) + '</td>' +
        '<td class="text-[10px] text-white/40 max-w-[200px] truncate" title="' + (a.reason || '').replace(/"/g, '&quot;') + '">' + (a.reason || '—') + '</td>' +
        '<td>' + agentActionButtons(a) + '</td>' +
        '</tr>';
    }).join('');
  }
  ...
  function fetchAgentActions() {
    fetch('/agent/actions')
      ...
      .catch(function (err) { console.error('Agent fetch error:', err); });
  }
  ...
  function agentGeneratePlan() {
    ...
    fetch('/agent/plan', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' })
      .then(function (r) { return r.json(); })
      .then(function (data) {
        if (data.error) { showAgentToast(data.error, 'error'); return; }
        showAgentToast((data.actions || []).length + ' actions proposed', 'success');
      ...
      .catch(function (err) { showAgentToast('Plan failed: ' + err, 'error'); })
      .finally(function () { DOM.btnAgentPlan && (DOM.btnAgentPlan.disabled = false); });
  }
  ...
  function agentSendCommand() {
    ...
    fetch('/agent/command', {
      ...
      .then(function (data) {
        if (data.error) { showAgentToast(data.error, 'error'); return; }
        showAgentToast((data.actions || []).length + ' actions from command', 'success');
      ...
      .catch(function (err) { showAgentToast('Command failed: ' + err, 'error'); });
  }

  // ─── LIFECYCLE ACTIONS (global for onclick) ───
  window.agentApprove = function (id) {
    fetch('/agent/approve', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action_id: id }) })
      .then(function (r) { return r.json(); })
      .then(function (data) {
        if (data.error) { showAgentToast(data.error, 'error'); return; }
      ...
      .catch(function (err) { showAgentToast('Approve failed: ' + err, 'error'); });
  };

  window.agentReject = function (id) {
    fetch('/agent/reject', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action_id: id, reason: 'Operator rejected via dashboard' }) })
      .then(function (r) { return r.json(); })
      .then(function (data) {
        if (data.error) { showAgentToast(data.error, 'error'); return; }
      ...
      .catch(function (err) { showAgentToast('Reject failed: ' + err, 'error'); });
  };

  window.agentExecute = function (id) {
    fetch('/agent/execute', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action_id: id }) })
      .then(function (r) { return r.json(); })
      .then(function (data) {
        if (data.error) { showAgentToast(data.error, 'error'); return; }
      ...
      .catch(function (err) { showAgentToast('Execute failed: ' + err, 'error'); });
  };

  window.agentRollback = function (id) {
    fetch('/agent/rollback', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action_id: id }) })
      .then(function (r) { return r.json(); })
      .then(function (data) {
        if (data.error) { showAgentToast(data.error, 'error'); return; }
      ...
      .catch(function (err) { showAgentToast('Rollback failed: ' + err, 'error'); });
  };

  // ─── AUTO-EXECUTE ───
  function agentAutoExecute() {
    showAgentToast('Running auto-execute...', 'info');
    fetch('/agent/auto', { method: 'POST', headers: { 'Content-Type': 'application/json' } })
      .then(function (r) { return r.json(); })
      .then(function (data) {
        var auto = (data.auto_executed || []).length;
        var held = (data.held_for_approval || []).length;
        showAgentToast(auto + ' auto-executed, ' + held + ' held for approval', auto > 0 ? 'success' : 'info');
        fetchAgentActions();
        fetchAgentAudit();
      })
      .catch(function (err) { showAgentToast('Auto-execute failed: ' + err, 'error'); });
  }

  // ─── DIFF VIEW ───
  window.agentShowDiff = function (id) {
    var action = agentState.actions.find(function (a) { return a.action_id === id; });
    ...
    html += '<div class="text-[10px] mono text-white/30 uppercase tracking-widest mb-2">Action Info</div>';
    html += '<div class="space-y-2">';
    html += '<div class="flex justify-between"><span class="text-[10px] text-white/40">ID</span><span class="text-[10px] mono text-white/60">' + (action.action_id || '—') + '</span></div>';
    html += '<div class="flex justify-between"><span class="text-[10px] text-white/40">Type</span>' + agentTypeBadge(action.action_type) + '</div>';
    html += '<div class="flex justify-between"><span class="text-[10px] text-white/40">Status</span>' + agentStatusBadge(action.status) + '</div>';
    html += '<div class="flex justify-between"><span class="text-[10px] text-white/40">Risk</span>' + agentRiskBadge(action.risk_level) + '</div>';
    html += '<div class="flex justify-between"><span class="text-[10px] text-white/40">Owner</span><span class="text-[10px] mono text-white/60">' + (action.owner_role || '—') + '</span></div>';
    html += '<div class="flex justify-between"><span class="text-[10px] text-white/40">Alert</span><span class="text-[10px] mono text-white/60">' + (action.alert_id || '—') + '</span></div>';
    html += '</div>';
    html += '<div class="mt-3"><span class="text-[10px] text-white/40">Reason</span><p class="text-[11px] text-white/70 mt-1">' + (action.reason || '—') + '</p></div>';
    html += '</div>';
    // Right: Params (before/after style)
    html += '<div class="space-y-3">';
    html += '<div class="text-[10px] mono text-white/30 uppercase tracking-widest mb-2">Parameters / Diff</div>';
    if (action.params && Object.keys(action.params).length > 0) {
      html += '<div class="bg-white/[0.03] rounded-xl p-4 space-y-1.5 border border-white/5">';
      Object.keys(action.params).forEach(function (key) {
        var val = action.params[key];
        if (typeof val === 'object') val = JSON.stringify(val);
        html += '<div class="flex justify-between"><span class="text-[10px] text-emerald-400/70 mono">+ ' + key + '</span><span class="text-[10px] mono text-emerald-400/90">' + val + '</span></div>';
      });
      html += '</div>';
    } else {
    ...
    if (action.execution_result) {
      html += '<div class="mt-3"><span class="text-[10px] text-white/40 block mb-1">Execution Result</span>';
      html += '<div class="bg-white/[0.03] rounded-xl p-4 border border-white/5 text-[10px] mono text-white/60">' + JSON.stringify(action.execution_result, null, 2).replace(/\n/g, '<br>').replace(/ /g, '&nbsp;') + '</div>';
      html += '</div>';
    }
    html += '</div>';
    html += '</div>';
  ...
  function fetchAgentAudit() {
    fetch('/agent/history')
      ...
      .then(function (data) {
        ...
        entries = entries.filter(function (e) { return e.event === 'approved' || e.event === 'rejected'; });
        if (entries.length === 0) {
          DOM.agentAuditLog.innerHTML = '<p class="text-[10px] mono text-white/20 text-center py-4">No audit entries yet.</p>';
          return;
        }
        var latest = entries.slice(0, 50);
        DOM.agentAuditLog.innerHTML = latest.map(function (e) {
          var eventColor = {
          ...
          if (!detail && snap.payload) {
            var p = snap.payload;
            detail = (p.item || p.ingredient || snap.action_type || '').replace(/_/g, ' ');
          }
          return '<div class="flex items-start gap-3 py-2 border-b border-white/[0.03] last:border-0">' +
            '<span class="text-[9px] mono text-white/20 shrink-0 w-16 mt-0.5">' + ts + '</span>' +
            '<span class="text-[9px] mono uppercase tracking-widest ' + color + ' shrink-0 w-28">' + (e.event || '').replace(/_/g, ' ') + '</span>' +
            '<span class="text-[10px] text-white/50 flex-1">' + (e.action_id ? e.action_id.slice(0, 8) + '...' : '') + (detail ? ' — ' + detail : '') + '</span>' +
            '</div>';
        }).join('');
      })
  ...
  function showAgentToast(message, type) {
    ...
    var colorMap = { info: 'border-indigo-500/40 bg-indigo-500/10', success: 'border-emerald-500/40 bg-emerald-500/10', error: 'border-red-500/40 bg-red-500/10' };
    var cls = colorMap[type] || colorMap.info;
    var toast = document.createElement('div');
    toast.id = 'agent-toast';
    toast.className = 'fixed bottom-6 right-6 z-50 px-5 py-3 rounded-xl border backdrop-blur-xl text-sm text-white/80 shadow-2xl transition-all ' + cls;
  ...
  if (DOM.autopilotToggle) {
    DOM.autopilotToggle.querySelectorAll('.autopilot-mode-btn').forEach(function (btn) {
      btn.addEventListener('click', function () {
        DOM.autopilotToggle.querySelectorAll('.autopilot-mode-btn').forEach(function (b) { b.classList.remove('active'); });
        btn.classList.add('active');
        agentState.autopilotMode = btn.getAttribute('data-mode');
        showAgentToast('Autopilot: ' + agentState.autopilotMode.toUpperCase(), 'info');
      });
    });
  }

  // ─── TABLE SORT ───
  document.querySelectorAll('#agent-queue-table th[data-sort]').forEach(function (th) {
    th.addEventListener('click', function () {
      var key = th.getAttribute('data-sort');
      if (agentState.sortKey === key) { agentState.sortAsc = !agentState.sortAsc; }
      else { agentState.sortKey = key; agentState.sortAsc = true; }
      renderAgentQueue();
    });
  });

  // ─── EVENT WIRING ───
  if (DOM.btnAgentPlan) DOM.btnAgentPlan.addEventListener('click', agentGeneratePlan);
  if (DOM.btnAgentCommand) DOM.btnAgentCommand.addEventListener('click', agentSendCommand);
  if (DOM.agentCommandInput) {
    DOM.agentCommandInput.addEventListener('keydown', function (e) {
      if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); agentSendCommand(); }
    });
  }
  if (DOM.btnAgentAuto) DOM.btnAgentAuto.addEventListener('click', agentAutoExecute);
  if (DOM.agentFilterStatus) DOM.agentFilterStatus.addEventListener('change', renderAgentQueue);
  if (DOM.agentFilterOwner) DOM.agentFilterOwner.addEventListener('change', renderAgentQueue);
  if (DOM.btnCloseDiff) {
